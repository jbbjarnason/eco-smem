#ifndef SMEM_FIXED_STRING_HPP
#define SMEM_FIXED_STRING_HPP
...
template <typename CharType, unsigned N> struct basic_fixed_string {
  ...
  requires(!std::same_as<
           other_char_type,
           char_type>) constexpr basic_fixed_string(const other_char_type (&foo)
                                                        [N + 1]) noexcept {
    std::transform(foo, foo + N + 1, data_,
                   [](other_char_type c) { return static_cast<char_type>(c); });
  }
  ...
  constexpr auto operator<=>(
      basic_fixed_string<char_type, N> const &) const noexcept = default;

  template <unsigned M>
  constexpr auto
  operator==(basic_fixed_string<char_type, M> const &r) const noexcept {
    return N == M && view() == r.view();
  }
};

template <typename char_type, unsigned N>
basic_fixed_string(char_type const (&str)[N])
    -> basic_fixed_string<char_type, N - 1>;

template <typename char_type, unsigned N, unsigned M>
consteval auto
concat_fixed_string(basic_fixed_string<char_type, N> l,
...
consteval auto concat_fixed_string(basic_fixed_string<char_type, N> l,
                                   basic_fixed_string<char_type, M> r,
                                   U... u) noexcept {
  return concat_fixed_string(l, concat_fixed_string(r, u...));
}

template <typename char_type, unsigned N, unsigned M>
consteval auto operator+(basic_fixed_string<char_type, N> l,
                         basic_fixed_string<char_type, M> r) noexcept {
